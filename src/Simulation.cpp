...
/*
 * OPEN MP VERSION OF FUNCTIONS
 */

void Simulation::calculateDensityMP() {
#pragma omp parallel for schedule(static)
  ...
  for (int i = 0; i < m_activeCount; i++) {
    ...
    if (i >= m_fluidCount && density < Config::REST_DENSITY) {
      density = Config::REST_DENSITY;
    }
    m_densities[i] = std::max(density, Config::REST_DENSITY * 0.2f);
  }
}
void Simulation::calculatePressureMP() {
#pragma omp parallel for schedule(static)
  for (int i = 0; i < m_activeCount; i++) {
    m_pressures[i] = Config::GAS_CONSTANT *
                     std::max(m_densities[i] - Config::REST_DENSITY, 0.f);
...
void Simulation::applyPressureAndViscosityForceMP() {
  ...
  constexpr float minDensityForForce = Config::REST_DENSITY * 0.3f;
  double neighbor_sum = 0.f;
#pragma omp parallel for schedule(static) reduction(+ : neighbor_sum)
  for (int i = 0; i < m_fluidCount; i++) {
    int local_neighbor = 0;
    Float2 viscForce = Float2(0.f, 0.f);
    Float2 pressureForce = Float2(0.f, 0.f);
...
void Simulation::applyForcesMP() {
  applyPressureAndViscosityForceMP();
  applyGravityMP();
}
void Simulation::integrateMP(float dt) {
...
#pragma omp parallel for schedule(static)                                      \
    reduction(+ : speed_sum, density_sum, pressure_sum)
  for (int i = 0; i < m_fluidCount; i++) {
    m_acceleration[i] = m_forces[i];
    m_velocities[i] += m_acceleration[i] * dt;
    if (m_velocities[i].length() > Config::maxSpeedAllowed) {
    ...
    m_positions[i] += m_velocities[i] * dt;
    speed_sum += m_velocities[i].length();
    density_sum += m_densities[i];
    pressure_sum += m_pressures[i];
  }
  avg_pressure = static_cast<float>(pressure_sum / m_fluidCount);
